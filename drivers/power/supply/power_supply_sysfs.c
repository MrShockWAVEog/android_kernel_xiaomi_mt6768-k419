...
static ssize_t power_supply_show_property(struct device *dev,
	...
	case POWER_SUPPLY_PROP_BATTERY_TYPE:
		if (value.intval < 0 ||
		    value.intval >= ARRAY_SIZE(power_supply_battery_type_text))
			return -EINVAL;
		ret = sprintf(buf, "%s\n",
			      power_supply_battery_type_text[value.intval]);
		break;
	case POWER_SUPPLY_PROP_MODEL_NAME ... POWER_SUPPLY_PROP_MANUFACTURER:
	case POWER_SUPPLY_PROP_CYCLE_COUNTS ... POWER_SUPPLY_PROP_SERIAL_NUMBER:
		ret = sprintf(buf, "%s\n", value.strval);
		break;
	default:
		ret = sprintf(buf, "%d\n", value.intval);
