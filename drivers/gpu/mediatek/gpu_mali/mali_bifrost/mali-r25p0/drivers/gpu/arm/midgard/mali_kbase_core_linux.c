	...
	char *const buf)
{
	...
	return sysfs_emit(buf, "%lu\n",
		(unsigned long)READ_ONCE(kbdev->total_gpu_pages) << PAGE_SHIFT);
}
	...
	char *const buf)
{
	...
	return sysfs_emit(buf, "%lu\n",
		(unsigned long)READ_ONCE(kbdev->dma_buf_pages) << PAGE_SHIFT);
}
...
int kbase_sysfs_init(struct kbase_device *kbdev)
{
	...
	kbdev->proc_sysfs_node = kobject_create_and_add("kprcs", &kbdev->dev->kobj);
	if (!kbdev->proc_sysfs_node) {
