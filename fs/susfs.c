					...
					char __user *buffer, int buflen)
{
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node, inode->i_ino) {
		if (!entry->reversed_lookup_only ||
		    entry->target_dev != inode->i_sb->s_dev)
			continue;
		ret = min_t(size_t, buflen, strlen(entry->info.redirected_pathname));
		if (copy_to_user(buffer, entry->info.redirected_pathname, ret))
			ret = -EFAULT;
		break;
	}
	srcu_read_unlock(&susfs_srcu_open_redirect, srcu_idx);
					     ...
					     char *tmp_buf, int buflen)
{
	...
	hash_for_each_possible_rcu(OPEN_REDIRECT_HLIST, entry, node, inode->i_ino) {
		if (!entry->reversed_lookup_only ||
			...
			continue;
		ret = min_t(size_t, buflen, strlen(entry->info.redirected_pathname));
		memcpy(tmp_buf, entry->info.redirected_pathname, ret);
		break;
	}
	srcu_read_unlock(&susfs_srcu_open_redirect, srcu_idx);
	return ret;
...
{
	/* Permission events in 4.19 may carry a path instead of a name. */
	if (!file_name && data_type == FSNOTIFY_EVENT_PATH) {
		const struct path *path = data;

		file_name = path->dentry->d_name.name;
	}
								...
								struct fsnotify_mark **out)
{
	...
	*out = m;
	/* The attached mark owns a reference until the group is destroyed. */
	fsnotify_put_mark(m);
	return 0;
}
...
static int susfs_sdcard_monitor_fn(void *data)
{
	...
	if (ret) {
		/* Use the same deferred cleanup and fail-open policy as thread failure. */
		if (!test_and_set_bit(0, &sdcard_cleanup_scheduled))
			queue_delayed_work(system_unbound_wq, &sdcard_cleanup_dwork, 0);
	}
