...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	/* Each RCU or stale-lookup retry starts a new pathname walk. */
	nd->state = 0;
#endif
...
skip_orig_flow:
	...
	if (dentry->d_inode) {
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
		/* d_alloc_parallel() can return a cached positive dentry too. */
		if (is_nd_state_open_last && !found_sus_path &&
		    susfs_is_inode_sus_path(dentry->d_inode)) {
			dput(dentry);
			dentry = NULL;
			found_sus_path = true;
			goto susfs_retry_open;
		}
#endif
...
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	/* Inspect the inode before ->atomic_open can open the original target. */
	goto no_open;
#elif defined(CONFIG_KSU_SUSFS_SUS_PATH)
...
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
	if (is_nd_state_open_last && !found_sus_path && dentry->d_inode &&
	    susfs_is_inode_sus_path(dentry->d_inode)) {
		dput(dentry);
		dentry = NULL;
		found_sus_path = true;
...
finish_open:
	...
	if (error)
		return error;
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	if (nd->dfd != -1 && !nd->susfs_redirected &&
	    SUSFS_IS_INODE_OPEN_REDIRECT_WITHOUT_UID_CHECK(nd->path.dentry->d_inode)) {
		struct filename *redirect;

		redirect = susfs_open_redirect_spoof_do_sys_openat(nd->path.dentry->d_inode);
		if (IS_ERR(redirect)) {
			error = PTR_ERR(redirect);
			goto out;
		}
		if (redirect) {
...
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	nd->susfs_redirect = NULL;
	nd->susfs_redirected = false;
#endif
